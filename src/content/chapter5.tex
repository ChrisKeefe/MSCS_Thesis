...
allows provenance to capture that seed value. q2\_feature\_table’s rarefy, for
...
stored as a .tsv in the Action’s UUID-named provenance directory. These .tsvs,
...
.tsv originally stored in its provenance, even if only one metadata sheet was
...
Pull request #464 on the qiime2 repository \parencite{qiime_2_development_team_pull_2016}
...
currently skips any Results whose root UUID is already contained in the DiGraph
being constructed, as all tracked predecessors of a node must also be in the
DiGraph when this occurs. A more granular approach can reduce redundant reads
...
The DiGraph structure of provenance lends itself to a number of graph operations
...
could be applied to UUID-based digraphs. In an academic context, the ability to
